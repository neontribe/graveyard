...
class CottageVocabManager {
  ...
  public static function addTermToVocabulary($machine_name, $term_name, $term_data = NULL, $parent = NULL) {
    ...
    if (self::taxonomyTermExists($machine_name, $term_name)) {

      $terms = self::getTermsFromName($machine_name, $term_name);
   ...
   */
  public static function createVocabularyFromAttribList($machine_name, $api_attribs, $vocab_definitions, $nested_parent_id = "group", $nested_item_id = "label") {
    foreach ($api_attribs as $attrib) {
      ...
      if (is_array($currentTerm)) {
        ...
        foreach ($currentTerm as $child_value) {
          ...
          self::addTermToVocabulary($machine_name, $cur_child_term, $field_data, self::getTermFromName($machine_name, $currentGroup));

        }
      }
      else {
        self::addTermToVocabulary($machine_name, $currentTerm, $field_data, self::getTermFromName($machine_name, $currentGroup));
      }
      ...
      // $groupExists = self::taxonomyTermExists($machine_name, $currentGroup);
      // Checks done internally (kept for reference)
      // Check if child element exists; if it doesn't then add it to the hierarchy.
      // $labelExists = self::taxonomyTermExists($machine_name, $currentTerm);.
    }

    return count($api_attribs);
  }

  /**
   * Ascertain whether a specific taxonomy_term already exists in the database.
   */
  public static function taxonomyTermExists($machine_name, $name) {
    $term = self::getTermFromName($machine_name, $name);
    if ($term != NULL) {
      return TRUE;
    }
   ...
   */
  public static function getTermFromName($machine_name, $name) {
   ...
   */
  public static function getTermsFromName($machine_name, $name) {
    $tid = taxonomy_get_term_by_name($name, $machine_name);
